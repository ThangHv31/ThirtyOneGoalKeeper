	...
	<div class="main">
		...
		<div class="content">
			...
			<div class="container col-11 bg-white">
				<div class="title">Sản Phẩm Mới Thêm Gần Đây</div>
					...
					<tbody>
						...
						<c:forEach items="${saleOrder}" var="saleorder" varStatus="loop">
							<tr>
								...
								<td><span id="_saleoder_status_${saleorder.id} }"> <c:choose>
											<c:when test="${saleorder.status }">
												<span class="badge badge-primary">Đang Xử Lí <i class="fas fa-shipping-fast"></i></span>
											</c:when>
											<c:otherwise>
												<span class="badge badge-warning">Đã Hoàn Thành <i class="fas fa-check"></i></span>
											</c:otherwise>
										</c:choose>
								</span></td>
								<td>
									<button class="btn btn-success" role="button"
										onclick="CompleteSaleorder(${saleorder.id})">Hoàn Thành</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
		...
		function CompleteSaleorder(saleorderId) {
			...
			// $ === jQuery
			$.ajax({
				url : "/admin/complete-saleorder?saleorderId=" + saleorderId,
				type : "post",
				...
				success : function(jsonResult) {
					if (jsonResult.code == 200) {
						...
						$("#_saleoder_status_" + saleorderId).html("<span class=\"badge badge-warning\">Đã Hoàn Thành <i class=\"fas fa-check\"></i></span>");
